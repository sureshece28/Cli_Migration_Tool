...
* @param {String} Template string using FreeMarker syntax, or compiled template name using jmarker-compile tool.
* @param {Object} Object data.
* @example
* var tplStr = 'Hello ${name}!', data = {'name': 'World'};
* // It the same as this form: jMarker(tplStr)(data);
* jMarker(tplStr, data); // 'Hello World!'
* 
* // Or we can save the template for further usage.
* var tpl = jMarker('Hello ${name}!'), data1 = {'name': 'World'}, data2 = {'name': 'jMarker'};
* 1. tpl(data1); // 'Hello World!'
* 2. tpl(data2); // 'Hello jMarker!'
* 
* // Or we can pre-compile the template using the compile tool.  
* 1. First use the jmarker-compile tool to generated JavaScript file for jMarker template.
* 	 jMarker.res('hello', function() {  ==Generated JavaScript Code== } );
* 2. Then invoke the jMarker like this.
* 	 jMarker('hello', {'name': 'World'});
* 
*/
var jMarker = (function () {


    // Utilities used by jMarker internal.
    /////////////////////////////////////////////////////////////////////////
    ...
    function _getType(value) {
        ...
        if (type === 'object') {
            var details = Object.prototype.toString.call(value);
    ...
    function _scriptsContent(tpls) {
        ...
        function parseKeyValueParis_(value) {
            ...
            while (j < count) {
                ...
                if (j === 0) {
                    ...
                    if (tmpLastIndex > 0) {
                        cache.push(jMarker.trim(part.substr(0, tmpLastIndex)));
        ...
        function startCommandImport(value) {
            ...
            if (regs && regs.length && regs.length === 3) {
                ...
                return 'var ' + paramName + '=jMarker.res(' + importFile + ')(null, true);';
            }
            return '';
        }
        ...
        function startCommandMacro(value) {
            ...
            for (j = 2; j < count; j += 2) {
                ...
                if (itemValue) {
                    ...
                    paramInitCode.push('if(!jMarker.exists(' + itemName + ')){' + itemName + '=' + itemValue + ';}');
        ...
        function pushStr(str) {
            // This parameter should not be trimed, for example: jMarker("${param1} ${param2}")
            // if (jMarker.trim(str) === '') {
            // return;
            // }
            // In switch command, there should not exist command after break and switch command.
            if (inSwitchCommand && (lastCommand === 'switch' || lastCommand === 'break')) {
                return;
            }
            str = str.replace(/'/g, "\\'");
            if (str !== '') {
        ...
        // macros - macro names inside this script block, also include function name.
        //		Note: There are slight difference between "macro" and "function" directive.
        //		"function" return undefined or object, while "macro" act as inline-content replacer.
        // assigns - assign names inside this script block.(Include local names, we don't distinguish assign from local variables)
        // lastCommand - Inside the switch command.
        var i, count = tpls.length, tpl, body = [], macros = [], assigns = [], inSwitchCommand = false, lastCommand = '';
    ...
    function _extendTokens(tokens, chars) {
        var i, j, count, token, countj, tmp;
        ...
        for (charKey in chars) {
            if (chars.hasOwnProperty(charKey)) {
                ...
                for (i = 0, count = charValue.length; i < count; i++) {
                    pos = charValue[i];
                    if (charKey === '??') {
                        tmp = 'jMarker.exists(typeof(' + tokens[pos[0]][1] + ')==="undefined"?undefined:' + tokens[pos[0]][1];
                        tokens[pos[0]][2] = tmp;
                        tokens[pos[1]][2] = ')';
                        tokens[pos[1] + 1][2] = '';
                    } else if (charKey === '!') {
                        tmp = 'jMarker.defaultValue(typeof(' + tokens[pos[0]][1] + ')==="undefined"?undefined:' + tokens[pos[0]][1];
                        if (pos.length === 2) {
                            tokens[pos[0]][2] = tmp;
                            tokens[pos[1]][2] = ')';
                        } else {
                        ...
                        // 12   punctuation     )
                        // ? [0,1,5,12]
                        tokens[pos[0]][2] = 'jMarker.builtins(' + tokens[pos[0]][1];
                        for (j = 1, countj = pos.length - 1; j < countj; j++) {
                            ...
                            // Modified by sanshi on 2011-8-5, to compatible with this syntax: ${x?string.number} ${x?string.currency} ${x?string.percent} ${x?string.computer}  
                            tokens[currentPos][2] = typeof (tokens[currentPos][2]) === 'undefined' ? tokens[currentPos][1] + '\'' : tokens[currentPos][2] + '\'';
                            // 5, 12
                        ...
                        tokens[pos[0]][2] = 'jMarker.sequence(' + tokens[pos[0]][1];
                        tokens[pos[1]][2] = ',';
                        tokens[pos[1] + 1][2] = '';
                        tokens[pos[2]][2] = tokens[pos[2]][1] + ')';
                    }
    ...
    function _builtin_contains(param, args) {
        return _builtin_index_of(param, args) !== -1;
    }
    ...
    function _builtin_xhtml(param, args) {
        return _builtin_html(param, args).replace(/'/ig, '&#39;');
    }
    ...
    function _builtin(param, name, args) {
        ...
        // Modified by sanshi on 2011-8-15, if the builtins are not supported, just ignore it.
        var fn = mappings[name];
        if (fn) {
    ...
    // Main function.
    /////////////////////////////////////////////////////////////////////////
    function jMarker(tpl, data) {
        if (typeof (data) === 'undefined') {
            var res = jMarker.res(tpl);
            if (!res) {
                //if (tpl.length > 200) {
                //throw 'jMarker template is too long, you should pre-compile the template.';
                //_log('You should pre-compile jMarker template first before usage.', tpl);
                //}
                res = _compile(tpl);
            }
            return res;
            ...
            try {
                return jMarker(tpl)(data);
            } catch (e) {
                _log('jMarker(tpl)(data) error:', e, tpl, data);
            }
        }
    }
    ...
    function _compile(tpl) {
        var rt = null;
        try {
            rt = new Function('__d', '__macro', _scripts(tpl));
        } catch (e) {
            rt = new Function('__d', '__macro', _scripts(''));
            _log('Compile jMarker template error:', e, tpl);
        }
        return rt;
    }
    ...
    // Set or get resource (compiled or not compiled)
    jMarker.res = function (key, tpl_fn) {
        if (typeof (tpl_fn) === 'undefined') {
            return _res[key];
        } else if (typeof (tpl_fn) === 'string') {
            _res[key] = _compile(tpl_fn);
        } else {
            _res[key] = tpl_fn;
        }
    };
    ...
    /////////////////////////////////////////////////////////////////////////
    // basePath, fileName, ['name', 'fileName'], ['name', ['fileName', 'fileName', 'fileName']]
    jMarker.precompile = function () {
        var compilePath = arguments[0], args = Array.prototype.slice.call(arguments, 1),
            i = 0, count = args.length, arg,
            name, files, fileContent = '';

        function removeEnds(str, ends) {
            var lastIndex = str.lastIndexOf(ends);
            if (lastIndex >= 0 && lastIndex + ends.length === str.length) {
        ...
        for (; i < count; i++) {
            ...
            name = removeEnds(name, '.txt');
            $.each(files, function (index, file) {
                // Send an synchronous AJAX to fetch template from server.
                $.ajax({
                    async: false,
                    dataType: 'txt',
                    url: compilePath + file,
                    cache: false,
    ...
    jMarker.trim = function (str) {
        str = str || '';
        return str.replace(/^(\s|\u00A0)+|(\s|\u00A0)+$/g, '');
    };

    jMarker.exists = function (value) {
        return typeof (value) !== 'undefined';
    };

    jMarker.defaultValue = function (value, defaultValue) {
        if (jMarker.exists(value)) {
            return value;
        } else {
            if (jMarker.exists(defaultValue)) {
                return defaultValue;
            } else {
    ...
    jMarker.sequence = function (start, end) {
        var i, result = [];
    ...
    jMarker._intermediate = _intermediate;
    jMarker._scripts = _scripts;
    jMarker._tokens = _tokens;
    jMarker._sepcialChars = _sepcialChars;
    jMarker._expression = _expression;
    jMarker._extendTokens = _extendTokens;
...
var i18n = {}; 
i18n.message = function(options){
  if (options.arguments) {
	return WWF.lang.getByLang(WWF.CONFIG.lang, options.code, options.arguments);
    //return WWF.lang.get.apply(options.code, options.arguments);
  } else {
    return WWF.lang.get(options.code);
  }
};